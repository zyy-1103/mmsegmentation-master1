...
img_norm_cfg = dict(  # Image normalization config to normalize the input images.
    mean=[123.675, 116.28, 103.53],  # Mean values used to pre-train the pre-trained backbone models.
    std=[58.395, 57.12, 57.375],  # Standard variance used to pre-train the pre-trained backbone models.
    to_rgb=True)  # The channel orders of image used to pre-train the pre-trained backbone models.
crop_size = (512, 1024)  # The crop size during train.
...
dist_params = dict(backend='nccl')  # Parameters to setup distributed train, the port can also be set.
log_level = 'INFO'  # The level of logging.
load_from = None  # load models as a pre-trained model from a given path. This will not resume train.
resume_from = None  # Resume checkpoints from a given path, the train will be resumed from the iteration when the checkpoint's is saved.
...
lr_config = dict(
    ...
    min_lr=0.0001,  # The minimum learning rate to stable the train.
