...
def test_build_dataset():
    ...
    # len(ann_dir) should be zero or len(images) when len(images) > 1
    ...
    # len(splits) should be zero or len(images) when len(images) > 1
    with pytest.raises(AssertionError):
        cfg = dict(
            type='CustomDataset',
            pipeline=[],
    ...
    # len(splits) == len(ann_dir) when only len(images) == 1 and len(
    # ann_dir) > 1
    with pytest.raises(AssertionError):
        cfg = dict(
            type='CustomDataset',
