...
def parse_args():
    ...
    parser.add_argument(
        'training_path', help='the train part of DRIVE dataset')
    parser.add_argument(
...
def main():
    ...
    mmcv.mkdir_or_exist(osp.join(out_dir, 'images', 'train'))
    mmcv.mkdir_or_exist(osp.join(out_dir, 'images', 'validation'))
    mmcv.mkdir_or_exist(osp.join(out_dir, 'annotations'))
    mmcv.mkdir_or_exist(osp.join(out_dir, 'annotations', 'train'))
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        ...
        now_dir = osp.join(tmp_dir, 'train', 'images')
        ...
        now_dir = osp.join(tmp_dir, 'train', '1st_manual')
        for img_name in os.listdir(now_dir):
            cap = cv2.VideoCapture(osp.join(now_dir, img_name))
            ret, img = cap.read()
            mmcv.imwrite(
                img[:, :, 0] // 128,
                osp.join(out_dir, 'annotations', 'train',
